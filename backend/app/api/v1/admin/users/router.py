...
async def create_user(
    user_in: UserCreate, _: Annotated[CurrentUser, Depends(operator_required)]
):
    """Создать нового пользователя."""
    logger.info(f"Attempting to create user with email: {user_in.email}")
    user = await async_db.add_user(
        user_in.email, user_in.password, user_in.name, user_in.role
    )
    if not user:
        logger.warning(
            f"Failed to create user. Email {user_in.email} might already exist."
        )
        raise HTTPException(
...
async def update_user(
    ...
    if user_id == current_user.id:
        ...
        raise HTTPException(
            status_code=400, detail="You cannot modify your own user account"
        )
...
async def delete_user(
    user_id: int, current_user: Annotated[CurrentUser, Depends(operator_required)]
):
    """Удалить пользователя."""
    if user_id == current_user.id:
        logger.warning(f"User {current_user.id} attempted to delete their own account.")
        raise HTTPException(
            status_code=400, detail="You cannot delete your own user account"
        )
