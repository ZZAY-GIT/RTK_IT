...
class Settings(BaseSettings):
    ...
    PROJECT_ROOT: pathlib.Path = pathlib.Path(
        __file__
    ).parent.parent.parent  # app/core → app → project_root

    # === База данных ===
    CONN_STR: str = Field(
        default="postgresql://warehouse_user:secure_password@localhost:5432/warehouse_db",
        alias="CONN_STR",
    )
    ...
    YANDEX_API_KEY: str = Field(
        default="AQVN2shaS1078O7IG0Imrbg-E_oaaOMX331y1e4B", alias="YANDEX_API_KEY"
    )
    ...
    YANDEX_MODEL: str = Field(
        default="gpt://b1grt4ckppkm30rupg8p/yandexgpt-lite", alias="YANDEX_MODEL"
    )
