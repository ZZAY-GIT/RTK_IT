...
class YandexGPTClient:
    ...
    def send_to_ai(self, inventory_data, historical_data):
        ...
        Example: """
            + """[{"product_id": "TEL-4567", "days_until_stockout": 9, "recommended_order": 100}, {"product_id": "TEL-8901", "days_until_stockout": 4, "recommended_order": 50}]
        """
        )
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "text": "You are a warehouse analysis assistant. Return ONLY a valid JSON array, no extra text or markdown.",
                },
    ...
    def get_prediction(self, inventory_data, historical_data, period_days=7):
        ...
        if not clean_predictions:
            ...
            return None
        logging.info(
            f"Successfully parsed {len(clean_predictions)} predictions from AI."
        )
        request_data = {
    ...
    def safe_parse_json(json_str: str):
        ...
        try:
            ...
            if end_index != -1:
                ...
                logging.warning(
                    "Закрывающий маркер ``` не найден. Ответ от AI, вероятно, обрезан."
                )
            ...
            try:
                ...
                logging.warning(
                    "JSON в ответе AI повреждён. Попытка восстановить все полные элементы."
                )
                ...
                try:
                    ...
                    if not isinstance(repaired_data, list) or not repaired_data:
                        logging.error(
                            "Восстановленные данные не являются валидным массивом."
                        )
                        return None
                    ...
                    logging.info(
                        f"Успешно восстановлено {len(repaired_data)} полных элементов из усечённого ответа."
                    )
                    return repaired_data

                except json.JSONDecodeError:
                    logging.error(
                        "Не удалось распарсить даже восстановленную JSON-строку."
                    )
                    return None
