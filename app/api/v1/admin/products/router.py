...
async def create_product(product: ProductCreate, _: Annotated[CurrentUser, Depends(access_level)]):
    """Создать новый продукт."""
    logger.info(f"Creating product with ID: {product.id}")
    product_id = await async_db.add_product(
        id=product.id, name=product.name, category=product.category,
        min_stock=product.min_stock, optimal_stock=product.optimal_stock
    )
    if not product_id:
...
async def get_product(product_id: str, _: Annotated[CurrentUser, Depends(access_level)]):
...
async def update_product(product_id: str, product_update: ProductUpdate, _: Annotated[CurrentUser, Depends(access_level)]):
    """Обновить данные продукта."""
    logger.info(f"Updating product with ID: {product_id}")
    update_data = product_update.model_dump(exclude_unset=True)
    product = await async_db.update_product(product_id, **update_data)
    if not product:
        logger.warning(f"Failed to update product with ID {product_id}. Product not found.")
        raise HTTPException(status_code=404, detail="Product not found")
    # ИСПРАВЛЕНО: Заменяем from_attributes на model_validate
    return ProductResponse.model_validate(product)

@router.delete("/{product_id}")
async def delete_product(product_id: str, _: Annotated[CurrentUser, Depends(access_level)]):
    """Удалить продукт."""
    logger.info(f"Deleting product with ID: {product_id}")
    success = await async_db.delete_product(product_id)
    if not success:
        logger.warning(f"Failed to delete product with ID {product_id}. Product not found.")
        raise HTTPException(status_code=404, detail="Product not found")
